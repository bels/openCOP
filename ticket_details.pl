...
if($authenticated == 1)
{
	...
	print  qq(<form action="update_ticket.pl" method="POST" id="update_form"><input type="hidden" name="ticket_number" value="$results->{'ticket'}"><label for="priority">Priority:</label><span id="priority" name="priority">$priorities{$results->{'priority'}}</span><br/>
		...
		<label for="status">Ticket Status:</label><span id="status" name="status">$ticket_statuses{$results->{'status'}}</span><label for="site">Site:</label><input type="text" id="site" name="site" value="$results->{'site'}"><label for="location">Location:</label><input type="text" id="location" name="location" value="$results->{'location'}"><br/>
