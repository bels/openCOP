...
if($authenticated == 1)
{
	...
	my $vars = {'title' => $title,'styles' => \@styles,'javascripts' => \@javascripts, 'company_name' => $config->{'company_name'},logo => $config->{'logo_image'}, section_list => $section_list, section_create_list => $section_create_list, scl => \@scl, is_admin => $user->is_admin(id => $id), reports => $reports};
